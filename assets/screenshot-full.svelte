<script>
  ...
  function cc(status) {
    if (status === 'none') return resolvedTheme === 'light' ? '#2563eb' : '#4B95E9';
    if (status === 'unknown') return resolvedTheme === 'light' ? '#a1a1aa' : '#71717a';
  ...
  const symbols = { clean: '●', behind: '◗', dirty: '◆', ahead: '▲', 'not cloned': '○', 'no upstream': '~', error: '✕' };
...
<div class="app">
  ...
  {#if showSettings}
    <div class="settings" transition:slide={{ duration: 150 }}>
      <div class="settings-row">
        ...
        <button class="settings-btn">Open in Editor</button>
      </div>
      <div class="settings-row">
      ...
      <div class="settings-row"><span class="settings-label">Run at startup</span><div class="theme-toggle"><button class="theme-btn theme-active">Off</button><button class="theme-btn">On</button></div></div>
      <div class="settings-row"><span class="settings-label">Version</span><span class="settings-value">1.2.1</span></div>
    </div>
  {/if}
  ...
  <section class="cards-row">
    {#each accounts as acct}
      ...
      <div class="card">
        <div class="card-top">
          <span class="card-dot" style="background: {cc(acct.credStatus)}"></span>
          <span class="card-provider">{acct.provider}</span>
          <span class="cred-badge cred-badge-{acct.credStatus === 'ok' ? 'ok' : 'err'}">{acct.cred}</span>
        </div>
        ...
        {#each source.repos as repo}
          ...
          <div class="repo-row" class:repo-row-clickable={repo.status === 'dirty'}
            ...
            <span class="status-badges">
              ...
              {:else if repo.status === 'behind'}
                ...
                <span class="sbadge" style="color:{sc('behind')}">↓{repo.behind}</span>
              {:else if repo.status === 'dirty'}
                <span class="status-pending">Pending</span>
                <span class="sbadge" style="color:{sc('dirty')}">✎{repo.modified}</span>
              {/if}
            </span>
          ...
          {#if expandedRepo === repoKey}
            <div class="repo-detail" transition:slide={{ duration: 150 }}>
              ...
              <div class="detail-section-title">Changed files <span class="sbadge" style="color:{sc('dirty')}">✎{repo.modified}</span></div>
...
<style>
  ...
  :global([data-theme="light"]) .cred-badge-err, :global([data-theme="light"]) .cred-badge-none { background: #dbeafe; border-color: #2563eb; color: #2563eb; }
