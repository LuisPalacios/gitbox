	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		for _, sourceName := range cfg.OrderedSourceKeys() {
			...
			for _, repoName := range source.OrderedRepoKeys() {
				...
				cloneErr := git.CloneWithProgress(cloneURL, dest, git.CloneOpts{}, func(p git.CloneProgress) {
				...
				if credType == "gcm" {
					credSection := fmt.Sprintf("credential.%s.username", strings.TrimSuffix(acct.URL, "/"))
					_ = git.ConfigSet(dest, credSection, acct.Username)
				}
