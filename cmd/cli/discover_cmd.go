...
Examples:
  ...
  gitboxcmd account discover github-personal --all
  gitboxcmd account discover github-personal --json
  gitboxcmd account discover github-personal --skip-forks --skip-archived`,
...
func runDiscover(cmd *cobra.Command, args []string) error {
	...
	if err != nil {
		if acct.DefaultCredentialType == "ssh" {
			...
			fmt.Fprintf(os.Stderr, "Store a PAT with: gitboxcmd account credential setup %s --token\n", accountKey)
		} else {
			fmt.Fprintf(os.Stderr, "No credentials available for account %q.\n", accountKey)
			fmt.Fprintf(os.Stderr, "Configure credentials with: gitboxcmd account credential setup %s\n", accountKey)
		}
	...
	if err != nil {
		if strings.Contains(err.Error(), "authentication failed") {
			fmt.Fprintf(os.Stderr, "\nCredentials rejected. Reconfigure with: gitboxcmd account credential setup %s\n", accountKey)
		}
		return err
	}
